...
\title{\textbf{ESC-50 环境声音检索与分类系统报告} \\ \large 《数字信号处理》期末项目报告}

\author{贾中豪，冉兴嵘，王胤博，张景超}
\date{\today}
...
模型 & 方法 & 预训练数据 & 准确率 \\
\midrule
\textbf{ResNet (Ours)} & Supervised (Scratch) & ESC-50(1.6K) & 75.00\% \\
Gemini Flash & Zero-shot LLM & - & 78.00\% \\
\textbf{Human Accuracy} & \textbf{Crowdsourcing} & \textbf{-} & \textbf{81.30\%} \cite{piczak2015esc} \\
PANNs & Transfer Learning & AudioSet (2M) & 90.50\% \\
CLAP (Zero-shot) & Zero-shot & Audio-Text (2.5M) & 91.50\% \\
AST & Transfer Learning & AudioSet (2M) & 95.00\% \\
\textbf{CLAP (Transfer)} & \textbf{Transfer Learning} & \textbf{Audio-Text (2.5M)} & \textbf{97.25\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{几点观察}
\begin{itemize}
    \item \textbf{超越人类听觉基准}：现代音频基础模型（CLAP, AST）的准确率稳定在90\%以上，显著优于人类的平均水平（81.30\%）。这证明了在大规模数据驱动下，模型专注于细粒度声学特征的辨识能力已在特定任务上超越了普通人类的直觉判断。
    \item \textbf{直觉与专家的界限}：自研模型（75.00\%）与通用多模态大模型Gemini（78.00\%）均徘徊在人类平均水平附近。这暗示了75\%-80\%的准确率可能代表了仅凭“听觉直觉”或“通用知识”所能达到的瓶颈，而要突破这一瓶颈迈向90\%以上的“专家领域”，则必须依赖大规模的专业声学领域预训练。
    \item \textbf{语义引导的优势}：CLAP模型凭借图文对比学习，在零样本（Zero-shot）条件下即取得了91.50\%的成绩，甚至超越了监督训练的PANNs模型。这一结果强有力地证明了：引入文本语义作为监督信号，能比单纯的标签分类更有效地帮助模型理解声音的本质含义。
\end{itemize}
...
\textbf{冉兴嵘 (DSP算法)}：负责项目的算法设计，从零实现了不依赖第三方库的FFT、STFT及梅尔频率倒谱系数(MFCC)算法，确保了底层信号处理的可控性。此外，通过与业界标准库的对比，验证了核心算法实现的数值精度。

\textbf{王胤博 (任务实验)}：负责项目实验与DSP相关算法机理验证。定量分析了帧长、帧移等关键参数对时频分辨率的影响。设计了适配Log-Mel谱的ResNet-18网络，并通过网格搜索实验全面评估了不同参数组合下的模型表现。

\textbf{贾中豪 (前沿模型研究)}：拓展项目的技术边界，引入了PANNs、AST和CLAP等业界前沿预训练模型。通过迁移学习和零样本分类实验，验证了大规模预训练模型的泛化能力，并完成了基于语义Embedding的高级检索实验。

\textbf{张景超 (集成与分析)}：探索Gemini多模态大模型在音频领域的应用，统筹项目的实验流程与数据可视化。负责综合分析各模块实验结果，梳理了从DSP到现代AI的技术演进脉络，并完成了项目报告与PPT的制作。
